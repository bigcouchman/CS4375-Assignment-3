...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Space-separated list of K values. Use at least 5 values for the assignment.",
    )
    parser.add_argument(
        "--max-iter",
        type=int,
...
def main() -> None:
    ...
    if len(unique_k_values) < 5:
        print(
            "Warning: the assignment asks for at least 5 different K values. "
            "Current unique K count is less than 5."
        )
    ...
    if not tweets:
        raise ValueError(
            "No tweets available after preprocessing. Try --keep-empty or a different file."
        )
